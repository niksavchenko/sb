...
class XWebDebugRouter extends CLogRoute {
	...
	private function checkAllowedIps($ip) {
		$isAllowed = false;
	...
	private function checkAjaxRequest($app) {
		return (($app instanceof CWebApplication) || $app->getRequest()->getIsAjaxRequest());
	}
	...
	public function processLogs($logs) {
		...
		// check IPs
		$isAllowed = $isAllowed && $this->checkAllowedIps($app->request->getUserHostAddress());
		// auto-disable debug toolbar based on response content type
		$isAllowed = $isAllowed && $this->checkContentTypeWhitelist();
		//Checking for an AJAX Requests
		$isAllowed = $isAllowed && $this->checkAjaxRequest($app);
